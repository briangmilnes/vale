...
lemma{:timeLimitMultiplier 6} lemma_CTR_Encrypt'_Is(inp : seq<Quadword>, key : seq<uint32>, alg : Algorithm, iv : uint64, init_ctr : uint64, n : uint64) 
  requires CTR_Encrypt_Req(inp, key, alg);
  requires 0 < |inp| < 0x1_0000_0000_0000_0000 - 1;
  ensures |CTR_Encrypt'(inp, key, alg, iv, init_ctr, n)| == |inp|;
    ...
    QuadwordXor(inp[i], AES_Encrypt(key, ctr_n(iv, init_ctr, BitwiseAdd64(n,i)), alg));
{
  ...
  if |inp| == 1 {
    ...
    lemma_CTR_Encrypt'_Is(inp[1..], key, alg, iv, init_ctr, BitwiseAdd64(n,1));
  }
}

/*

lemma{:timeLimitMultiplier 6} lemma_CTR_Encrypt_Is'(inp : seq<Quadword>, key : seq<uint32>, alg : Algorithm, iv : uint64, init_ctr : uint64) 
  requires CTR_Encrypt_Req(inp, key, alg);
  requires 0 < |inp| < 0x1_0000_0000_0000_0000 - 1;
...
lemma{:timeLimitMultiplier 3} lemma_CTR_Encrypt_Is(inp : seq<Quadword>, key : seq<uint32>, alg : Algorithm, iv : uint64, init_ctr : uint64) 
  requires CTR_Encrypt_Req(inp, key, alg);
  requires 0 < |inp| < 0x1_0000_0000_0000_0000 - 1;
  ensures |CTR_Encrypt(inp, key, alg, iv, init_ctr)| == |inp|;
  ensures forall i : nat :: i < |inp| ==> 
...
{
  lemma_CTR_Encrypt_length(inp);
  lemma_CTR_Encrypt_Is'(inp, key, alg, iv, init_ctr);
}
*/
